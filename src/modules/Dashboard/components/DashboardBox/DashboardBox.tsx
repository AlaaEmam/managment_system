...
const DashboardBox: React.FC<DashboardBoxProps> = ({ title, description, cards }) => {
    return (
        <div className='dashboard-box col-12 col-md-6'>
                ...
                {cards.map((card, index) => (
                    <div className="card" key={index}>
                        <div className={`icon-card ${card.iconBgClass}`}>
                            <img className='icon' src={card.icon} alt={`${card.label} Icon`} />
                        </div>
